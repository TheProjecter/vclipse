...
public class ModelChange extends CompositeChange {
	...
	public RefactoringStatus refactor(final IProgressMonitor pm) throws CoreException {
		...
		if(isEnabled()) {
			...
			final Map<ModelEntryChange, CoreException> exceptions = Maps.newHashMap();
			context.getDocument().modify(new IUnitOfWork<Object, XtextResource>() {
				@Override
				public Object exec(XtextResource state) throws Exception {
					for(Change change : changes) {
						if(change.isEnabled()) {
							if(change instanceof ModelEntryChange) {
								ModelEntryChange scc = (ModelEntryChange)change;
								scc.isValid(pm);
								scc.perform(pm);
								sm.worked(1);
							}							
			...
			if(!exceptions.isEmpty()) {
				...
				for(Entry<ModelEntryChange, CoreException> exception : exceptions.entrySet()) {
					final ModelEntryChange scc = exception.getKey();
	...
	private void recordSourceCodeChanges(IProgressMonitor pm, IRefactoringUIContext previewContext) {
		...
		for(Entry<EObject, EList<FeatureChange>> entry : objectChanges.entrySet()) {
			...
			for(FeatureChange featureChange : featureChanges) {
				...
				ModelEntryChange scc = new ModelEntryChange();
				scc.register(existingEntry, refactored, featureChange);
